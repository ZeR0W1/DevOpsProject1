...
| `evidence/p4/` | Non-secret Assignment 4 CI/CD and runtime evidence screenshots |
...
```bash
export KUBECONFIG="$PWD/Ansible-modules-01/recovery/target-kubeconfig"
```
...
obtain explicit approval. Delete exactly one application pod, confirm that its
Deployment creates a replacement with a different name and restores both replicas
to Ready on the same immutable image, inspect namespace events sorted by creation
time, then verify `/`, `/health`, and `/machines` through the public frontend.
