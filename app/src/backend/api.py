...
def translate_enum_field(record: dict, field: str, enum_type) -> None:
    """Replace a known enum value with its display name in place."""
    value = record.get(field)
    if value is None:
        return
    try:
        record[field] = enum_type(value).name
    except ValueError:
        pass
...
def translate_machine_for_display(machine: dict) -> dict:
    ...
    translate_enum_field(cpu, "architecture", CPUArchitecture)
    translated["cpu"] = cpu

    os_config = dict(translated.get("os", {}))
    translate_enum_field(os_config, "name", OSName)
    translated["os"] = os_config

    disks = []
    for disk in translated.get("disks", []):
        disk_copy = dict(disk)
        translate_enum_field(disk_copy, "type", DiskType)
