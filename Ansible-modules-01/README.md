...
- `playbooks/create/setup/setup_local_environment.yml` — one-time interactive
  clone/fork SCM selection, webhook-token access check, email/CIDR setup,
...
FULL CD. Setup detects and confirms the controller clone's GitHub HTTPS origin and
checked-out branch, or accepts another GitHub HTTPS repository and branch that the
operator can administer. The playbook verifies the encrypted token can read that
repository's webhooks before AWS/Terraform work, then displays the repository,
branch, AWS account, region, billable resources, and retained-state decision before
requiring the exact `CREATE` confirmation. Jenkins job SCM and `REPO_URL`/ref
parameter defaults all use the same local selection.
...
- `.vault-password` and `vault/local-environment.yml` are ignored mode-`0600`
  ...
  credentials.
- `vars/project.local.yml` is an ignored mode-`0600` non-secret SCM selection
  generated by setup. Use `vars/project.local.example.yml` as its schema when
  deliberately changing the Jenkins/webhook repository or watched branch, then
  rerun the guarded lifecycle to validate and reseed that selection.
